...
class Experiment(object):
    def __init__(self, name):
        self.batch_size = 32
        self.__train_loader = get_data_loader( data_path+'/train_captions.pkl', data_path+'/ResNet101_train.hdf5',batch_size=self.batch_size,shuffle=True)
        self.__val_loader = get_data_loader( data_path+'/val_captions.pkl', data_path+'/ResNet101_val.hdf5',batch_size=self.batch_size,shuffle=True)
        self.__test_loader = get_data_loader( data_path+'/test_captions.pkl', data_path+'/ResNet101_test.hdf5',batch_size=self.batch_size,shuffle=False)
    ...
    def __train(self):
        ...
        for i, (videos, captions, lengths, video_ids,cap_mask) in enumerate(self.__train_loader):
            ...
            features = self.__encoder_model(inputs,train_labels)
    ...
    def __val(self):
        ...
        with torch.no_grad():
            for i, (videos, captions, lengths, video_ids,cap_mask) in enumerate(self.__val_loader):
                ...
                features = self.__encoder_model(inputs,val_labels)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for iter, (images, captions, lengths, video_ids,cap_mask) in enumerate(self.__test_loader):
                ...
                features = self.__encoder_model(inputs,test_labels)
