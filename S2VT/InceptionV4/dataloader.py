...
class V2TDataset(data.Dataset):
    ...
    def __init__(self, cap_pkl, feature_h5,begin,total):
        ...
        self.video_feats, self.video_ids,self.captions = self.get_video_feats(feature_h5,begin,total) #filename = data_path+'/ResNet101_test.hdf5'

    def get_video_feats(self,filename,begin,total):  #1200
        ...
        with h5py.File(filename, "r") as f:
            ...
            for i in range(0,total): #100
              ...
              if "train" in filename:
                ...
                start = begin+1 #1201
                assigned_id = start + i #1201 + 0
              sum+=len(video_metadata)
              #print("{}. Video_id = {}, # frames = {},assigned_id = {}".format(i,video_id,len(video_metadata),assigned_id))
              #indices = [j for j in range(0,len(video_metadata),2)] # sampling every 5th frame as per 2015 paper (subhashini)
              results[assigned_id] = video_metadata#[indices]
              result_ids.append(assigned_id)
              caption_indices = self.id2idx[assigned_id]#[0:5] #[0,1,2,3,4]
...
def get_data_loader(cap_pkl, feature_h5,begin=0,total=0, batch_size=10, shuffle=False, num_workers=1, pin_memory=True,drop_last=True):
    v2t = V2TDataset(cap_pkl, feature_h5,begin,total)
