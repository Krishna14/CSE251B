...
class MiCTResNet(nn.Module):
    ...
    def __init__(self, block, layers, dropout, version, n_classes, **kwargs):
        """
            ...
            :param layers: the number of blocks for each for each of the four feature depth.
        ...
        self.conv2 = nn.Conv3d(3, 64, kernel_size=(7, 7, 7),stride=(self.t_strides[0], 2, 2),padding=0, bias=False)
        self.bn2 = nn.BatchNorm3d(64)
        self.maxpool2 = nn.MaxPool3d(kernel_size=3, stride=2, padding=1)
        self.relu = nn.ReLU(inplace=True)

        self.layer1 = MiCTBlock(block, self.inplanes, 64, layers[0], stride=(self.t_strides[1], 1))
        self.layer2 = MiCTBlock(block, self.layer1.inplanes, 128, layers[1], stride=(self.t_strides[2], 2))
        self.layer3 = MiCTBlock(block, self.layer2.inplanes, 256, layers[2], stride=(self.t_strides[3], 2))
        self.layer4 = MiCTBlock(block, self.layer3.inplanes, 512, layers[3], stride=(self.t_strides[4], 2))
        ...
        # for each module in the range of all modules
...
# Computing either the BasicBlock or Bottleneck
    ...
    def __init__(self, block, inplanes, planes, blocks, stride=(1, 1)):
        ...
        if stride[1] != 1 or inplanes != planes * block.expansion:
            ...
            downsample = nn.Sequential(
                nn.Conv2d(inplanes, planes * block.expansion, kernel_size=1, stride=stride[1], bias=False),
...
def get_mictresnet(backbone, version, dropout=0.5, n_classes=101, pretrained=False, **kwargs):
    ...
    if backbone == 'resnet18':
        ...
        model = MiCTResNet(Bottleneck, [3, 4, 6, 3], dropout, version, n_classes, **kwargs)
        if pretrained:
            model.transfer_weights(model_zoo.load_url(model_urls['resnet50']))
    else:
